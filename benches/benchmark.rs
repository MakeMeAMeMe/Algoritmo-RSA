...
use trabalho::encriptador::gerador_chaves::random_prime::{teste_primalidade, generate_prime};
...
fn primalidade_benchmarck(c: &mut Criterion) {
    let mut group = c.benchmark_group("Teste Primalidade");
    ...
    let tamanho_max = 1024u64;
    for tamanho in (tamanho_min..=tamanho_max).step_by(64usize) {
        let num = generate_prime(tamanho);
        group.bench_with_input(
            BenchmarkId::new("Teste Primalidade ", &tamanho),
            &num,
            |b, num| {
                b.iter(|| teste_primalidade(&num, 20));
            },
        );
    }
    group.finish();
